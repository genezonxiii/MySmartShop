...
class GuideEng extends Component {
  constructor(props) {
    super(props)
    this.state = {
      entry: [],
  ...
  loadEntry() {
    db.transaction((tx) => {
      tx.executeSql('SELECT entrypointid, entrypointdesc FROM tb_guide where language="english" group by entrypointid, entrypointdesc', 
        [], (tx, results) => {
